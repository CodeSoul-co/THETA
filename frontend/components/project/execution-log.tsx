  ...
  return <details className="rounded-xl border bg-white p-4" open>
    ...
    <div className="mt-3 space-y-3">
      {states.map(state => <div key={state.id} className="rounded-lg bg-slate-50 p-3 text-sm">
        ...
        {(state.diagnostics?.message || state.error) && <div className="mt-2 rounded-lg border border-red-200 bg-red-50 p-3 text-red-700"><SetupError error={state.diagnostics?.message || state.error!} /></div>}
