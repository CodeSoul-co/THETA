...
import { artifactLabel, fieldLabel, statusLabel, systemText } from "@/lib/presentation"
  ...
  return (
    <div className={css.resultsPreview}>
      <div className={css.resultsSummary}>
        <div><strong>{statusLabel(results.researchStatus ?? results.status)}</strong><span>{systemText(results.message)}</span></div>
        <a className={css.archiveButton} href={resultArchiveUrl(runId)}>{t('download')} ↓</a>
      </div>
      {results.visualizations.map((visualization) => {
        const attachment: WebAttachment = { kind: 'visualization', id: visualization.id, label: artifactLabel(visualization.label, "figure") }
        return (
          <article
          ...
          >
            {visualization.format === 'image'
              ? <img src={resultAssetUrl(runId, visualization.relativePath)} alt={artifactLabel(visualization.label, "figure")} loading="lazy" />
              : <iframe src={resultAssetUrl(runId, visualization.relativePath)} title={artifactLabel(visualization.label, "figure")} sandbox="allow-scripts" />}
            <div>
              <strong>{artifactLabel(visualization.label, "figure")}</strong>
      ...
      {Object.keys(results.metrics).length > 0 && (
        <div className={css.metricGrid}>
          {Object.entries(results.metrics).slice(0, 12).map(([key, value]) => (
            <button key={key} type="button" onClick={() => onAttach({ kind: 'metric', id: key, label: fieldLabel(key) })}>
              <span>{fieldLabel(key)}</span><strong>{typeof value === 'number' ? value.toFixed(3) : String(value)}</strong>
            </button>
          ))}
        </div>
      )}
