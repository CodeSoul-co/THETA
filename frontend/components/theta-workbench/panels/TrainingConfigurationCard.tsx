...
export function TrainingConfigurationCard({ runId, interaction, onApproved }: {
  ...
  const initialConfig = useMemo(() => data ? { ...configFromPlans(data.plans), ...(data.inputKind === 'text' ? { textColumn: 'text', timeColumn: undefined, labelColumn: undefined, covariates: [] } : {}), stopwords: data.stopwords } : undefined, [data])
  ...
  return <section className={css.card}>
    ...
    <div className={css.body}>
      ...
      {data?.inputKind === 'text' && <p className={css.scopeNotes}>已直接读取正文，无需选择数据列。</p>}
    ...
    {data && initialConfig && <AnalysisConfigPanel key={key} open={open && interaction.status !== 'running'} onOpenChange={changeOpen} projectKey={key}
      datasetName={data.datasetName} datasetSizeBytes={data.datasetSizeBytes} initialConfig={initialConfig} columns={data.inputKind === 'text' ? undefined : data.columns} service={service}
