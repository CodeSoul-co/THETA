...
function sanitizeUrl(url: string): string {
  ...
  if (/^\/api\/(?:v3|v1\/agent)\/runs\/chat-[a-f0-9-]+\/artifacts\/[a-f0-9]{64}\/download$/u.test(url)) return url
  try {
...
function renderSafeLink(href: string, children: ReactNode[], key: Key): ReactNode {
  ...
  const internalApi = safeHref.startsWith('/api/v3/') || safeHref.startsWith('/api/v1/agent/')
  const external = internalApi || ['http:', 'https:'].includes(new URL(safeHref).protocol)
  return (
    <a
      key={key}
      href={safeHref}
