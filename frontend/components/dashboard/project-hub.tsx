...
export interface Project {
  ...
  /** 一行摘要，例如「153 图 · 38 表」 */
  summary?: string
  /** Project lifecycle is owned by the Agent project API rather than the legacy dashboard API. */
  readOnly?: boolean
}
...
export function ProjectHub({ onProjectSelect, onNewProject, onDeleteProject, onBatchDelete, onRefresh, projects = [], isLoading }: ProjectHubProps) {
  ...
  const projectList = projects.filter((p) => p.id && !p.id.startsWith("new-") && !p.readOnly)
        ...
        {!isLoading && (
          ...
          >
            ...
            {projects.map((project) => {
              ...
              // 卡片很窄：标题优先显示「·」后的短名（例如模型名），完整名字放到 title 里。
              const displayName = project.name.includes("·") ? (project.name.split("·").pop() ?? project.name).trim() : project.name
              ...
              return (
                ...
                >
                  ...
                  <CardContent className="p-4 relative">
                    ...
                    {/* 项目管理：更多 -> 删除 */}
                    {onDeleteProject && !project.readOnly && (
                    ...
                    <div className={`flex items-start justify-between gap-2 mb-3 pr-7 ${canSelect ? "pl-7" : ""}`}>
                      <div className="flex-1 min-w-0">
                        <h3
                          title={project.name}
                          className="truncate font-semibold text-slate-800 group-hover:text-slate-900 text-sm leading-snug"
                        >
                          {displayName}
                        </h3>
                        {displayName !== project.name && (
                          <p className="mt-0.5 truncate text-[11px] text-slate-400" title={project.name}>{project.name.split("·")[0].trim()}</p>
                        )}
                      </div>
