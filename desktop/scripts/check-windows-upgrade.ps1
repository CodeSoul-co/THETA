...
if ($Phase -eq 'download') {
  ...
  & gh release download desktop-v0.3.7 --repo CodeSoul-co/THETA --pattern 'THETA-0.3.7-win-x64.exe' --dir $baselineFolder
...
function Install-Theta($installer, $folder) {
  ...
  Wait-ThetaProcess $process 1200000 "Installer $installer"
...
if ($Phase -eq 'baseline') {
  Install-Theta (Join-Path $baselineFolder 'THETA-0.3.7-win-x64.exe') $oldFolder
