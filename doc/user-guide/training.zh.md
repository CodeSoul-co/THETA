...
手动模式的执行日志约每 2.5 秒更新，展示模型实际输出的训练轮次、批次或迭代次数、损失值以及提前停止信息。各模型提供的指标不同；阶段百分比不代表剩余时间。

嵌入阶段会显示完成的文本数或词表批次；本地和云端文本编码还会显示已编码的文本块数。长文本的全部分块处理成功后才计为完成，失败的云端请求不会增加完成数。

取消勾选“跟随最新”即可停留查看之前的记录。训练完成后，在结果页打开“执行日志”，可查看所选模型的近期历史。完整训练日志保存在本地任务目录。
