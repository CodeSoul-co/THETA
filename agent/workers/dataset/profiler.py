...
TEXT_NAMES = {'text', 'content', 'body', 'message', 'review', 'comment', 'title', 'abstract', 'description', '正文', '内容', '文本', '标题', '摘要'}
TIME_HINTS = ('time', 'date', 'created', 'updated', 'timestamp', 'year', 'month', '时间', '日期', '年份', '月份')
...
def _parse_datetime(value: str) -> datetime | None:
    value = value.strip().replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
...
def _time_coverage(rows: list[dict[str, Any]], columns: list[str]) -> dict[str, str | None]:
    values = sorted(parsed.isoformat() for row in rows for column in columns
                    if row.get(column) is not None and (parsed := _parse_datetime(str(row[column]))) is not None)
