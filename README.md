...
[![Desktop](https://img.shields.io/badge/Desktop-macOS%20%7C%20Windows-blue?style=flat-square)](https://github.com/CodeSoul-co/THETA/releases/tag/desktop-v0.3.8)
...
## Choose how to use THETA

Use **the Web workbench, CLI Agent, or desktop app**, with an optional Skill for other agents. This repository maintains the local edition: the workbench and compute services run on your computer, where projects and results are stored. You can configure your own cloud LLM and Embedding APIs.

| Entry point | Best for | Get started |
| --- | --- | --- |
| **Desktop · Mac / Windows** | Installing and using THETA without setting up a development environment | [Download an installer](https://github.com/CodeSoul-co/THETA/releases/tag/desktop-v0.3.8); Python and compute dependencies are bundled. No separate Python, Node.js, or Conda installation is needed |
...
### Desktop downloads · 0.3.8 preview

From 0.3.8, use **Settings → App updates** for future downloads. Windows supports confirmed restart-to-install; the unsigned Mac preview downloads the DMG in the app. Earlier versions need one manual installation to enable this feature.
...
| System | Installer |
| --- | --- |
| macOS · Apple Silicon (M-series) | [Download DMG](https://github.com/CodeSoul-co/THETA/releases/download/desktop-v0.3.8/THETA-0.3.8-mac-arm64.dmg) |
| Windows · x64 | [Download EXE installer](https://github.com/CodeSoul-co/THETA/releases/download/desktop-v0.3.8/THETA-0.3.8-win-x64.exe) |
...
```bash
./theta-web start     # Build, check, and start the frontend and both APIs in the background
./theta-web status    # Show service health, logs, and data directories
./theta-web restart   # Rebuild and restart after updating code
./theta-web stop      # Stop web services; keep projects and training results
./theta-web check     # Build and check the Worker without starting training
```

Open **<http://127.0.0.1:4320/workbench?mode=conversation>** and switch between Conversation and Manual at the top. Closing the terminal does not stop the services; run `./theta-web start` again after rebooting your computer.

First-time setup requires **Node.js ≥ 22.13, pnpm, the Python compute environment, and private `agent/.env.local` configuration** ([Agent setup](agent/README.md)). Install dependencies once:
