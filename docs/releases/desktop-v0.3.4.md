# THETA 0.3.4 · Desktop Preview

**English** | [中文](https://github.com/CodeSoul-co/THETA/blob/main/docs/releases/desktop-v0.3.4.zh.md)
...
| --- | --- | --- |
| macOS Apple Silicon | [DMG](https://github.com/CodeSoul-co/THETA/releases/download/desktop-v0.3.4/THETA-0.3.4-mac-arm64.dmg) | Open the DMG and drag THETA into Applications |
| Windows x64 | [EXE](https://github.com/CodeSoul-co/THETA/releases/download/desktop-v0.3.4/THETA-0.3.4-win-x64.exe) | Run the installer |

The macOS ZIP is an alternative app archive; use the DMG for a standard installation. [SHA256SUMS.txt](https://github.com/CodeSoul-co/THETA/releases/download/desktop-v0.3.4/SHA256SUMS.txt) contains checksums for each installer. Source archives do not include the desktop runtime.
...
- The homepage no longer shows pricing plans. Its feature and tutorial images are actual THETA screenshots, with a larger preview on click.
- Fixed homepage screenshots failing to load in the desktop app’s authenticated session.
- Manual-mode execution logs show live epochs, loss values, and embedding document or batch progress when the model reports them. Logs remain available in the results view after completion and restart.

## Upgrade from an earlier version

The desktop app includes a local copy of the website. It does not download homepage changes automatically. Quit THETA and install 0.3.4 to receive this homepage and the new execution logs.

On macOS, replace THETA in Applications with the app from the DMG. On Windows, run the new EXE installer; it removes the previous application files and refreshes UI caches while preserving projects, results, models, and settings.
