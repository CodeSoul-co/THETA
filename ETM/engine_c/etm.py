...
class ETM(nn.Module):
    ...
    def forward(
        ...
        if compute_loss:
            # Normalize BOW targets to get probability distribution
            bow_sum = bow_targets.sum(dim=1, keepdim=True)
            bow_prob = bow_targets / (bow_sum + 1e-10)
            
            # 1. Reconstruction loss: negative log likelihood
            recon_loss = -torch.sum(bow_targets * log_word_dist, dim=-1).mean()
            
            # 2. KL divergence between BOW probability and predicted word distribution
            # KL(P||Q) = sum(P * log(P/Q)) = sum(P * log(P) - P * log(Q))
            kl_bow_loss = torch.sum(bow_prob * (torch.log(bow_prob + 1e-10) - log_word_dist), dim=-1).mean()
            
            # 3. KL divergence for variational inference (from encoder)
            kl_loss = kl_theta_loss + kl_weight * kl_bow_loss
            
            output['recon_loss'] = recon_loss
            output['kl_theta_loss'] = kl_theta_loss
            output['kl_bow_loss'] = kl_bow_loss
