...
def run_visualization(
    ...
    from visualization.topic_visualizer import TopicVisualizer, load_etm_results
    ...
    # Create visualizer
    visualizer = TopicVisualizer(output_dir=viz_dir)
    
    # Generate visualizations
    prefix = f"{config.data.dataset}_{config.embedding.mode}"
    
    # Save topic words to dedicated folder (skip wordcloud generation)
    ...
    # Save topic words as readable text file
    ...
    logger.info("Generated topic similarity heatmap")
    
    # Generate combined wordcloud (all words in one big wordcloud)
    try:
        visualizer.visualize_combined_wordcloud(
            results['topic_words'],
            num_words=50,
            filename=f"combined_wordcloud_{prefix}.png"
        )
        logger.info("Generated combined wordcloud")
    except Exception as e:
        logger.warning(f"Failed to generate combined wordcloud: {e}")
    
    # Generate topic wordclouds grid (each topic in a grid)
    try:
    ...
    # Generate topic words bar chart (all topics in one image)
    ...
    logger.info("Generated topic words bar chart")
    ...
    logger.info("Generated topic proportions visualization")
    
    # Generate metrics visualization if evaluation results exist
    logger.info(f"Looking for metrics in: {config.evaluation_dir}")
    eval_files = sorted(Path(config.evaluation_dir).glob("metrics_*.json"), reverse=True)
    logger.info(f"Found {len(eval_files)} metrics files: {eval_files}")
    if eval_files:
        try:
            ...
            visualizer.visualize_metrics(
                metrics,
                filename=f"metrics_{prefix}.png"
            )
            logger.info("Generated metrics visualization")
        except Exception as e:
            logger.warning(f"Failed to generate metrics visualization: {e}")
    
    # Generate t-SNE/UMAP visualization of topic and word embeddings
    topic_emb_path = os.path.join(config.model_dir, f"topic_embeddings_{timestamp}.npy")
    vocab_emb_path = os.path.join(config.bow_dir, "vocab_embeddings.npy")
    vocab_path = os.path.join(config.bow_dir, "vocab.txt")
    
    if os.path.exists(topic_emb_path) and os.path.exists(vocab_emb_path) and os.path.exists(vocab_path):
        topic_embeddings = np.load(topic_emb_path)
        word_embeddings = np.load(vocab_emb_path)
        with open(vocab_path, 'r', encoding='utf-8') as f:
            vocab = [line.strip() for line in f]
        
        # t-SNE 2D visualization
        try:
            visualizer.visualize_topic_word_embeddings(
                ...
                top_k_words=10,
                n_components=2,
                filename=f"topic_word_tsne_2d_{prefix}.png"
            )
            logger.info("Generated t-SNE 2D topic-word embedding visualization")
        except Exception as e:
            logger.warning(f"Failed to generate t-SNE 2D visualization: {e}")
        ...
        try:
            visualizer.visualize_topic_word_embeddings(
                ...
                top_k_words=10,
                n_components=2,
                filename=f"topic_word_umap_2d_{prefix}.png"
            )
            logger.info("Generated UMAP 2D topic-word embedding visualization")
        except Exception as e:
            logger.warning(f"Failed to generate UMAP 2D visualization: {e}")
    else:
        logger.warning("Topic or word embeddings not found, skipping embedding visualization")
    
    # Generate pyLDAvis interactive visualization
    from visualization.topic_visualizer import generate_pyldavis_visualization
    
    # Load BOW matrix for pyLDAvis
    bow_path = os.path.join(config.bow_dir, "bow_matrix.npz")
    vocab_path = os.path.join(config.bow_dir, "vocab.txt")
    ...
    if os.path.exists(bow_path) and os.path.exists(vocab_path):
        ...
        pyldavis_path = os.path.join(viz_dir, f"pyldavis_{prefix}.html")
        ...
        logger.info(f"Generated pyLDAvis interactive visualization: {pyldavis_path}")
    else:
        logger.warning("BOW matrix or vocab not found, skipping pyLDAvis")
    
    # Generate pyLDAvis style static visualization
    try:
        ...
        logger.warning(f"Failed to generate pyLDAvis style visualization: {e}")
    
    # Generate topic network visualization
    try:
        from visualization.topic_network import TopicNetworkVisualizer
        network_viz = TopicNetworkVisualizer(
            theta=results['theta'],
            topic_words=results['topic_words'],
            output_dir=viz_dir
        )
        network_viz.plot_topic_network(
            correlation_threshold=0.3,
            filename=f"topic_network_{prefix}.png"
        )
        logger.info("Generated topic network visualization")
    except Exception as e:
        logger.warning(f"Failed to generate topic network visualization: {e}")
    
    # Generate document-topic t-SNE visualization
    try:
        ...
        logger.info("Generated document-topic t-SNE visualization")
    except Exception as e:
        logger.warning(f"Failed to generate doc_topics visualization: {e}")
    
    # Generate training history / perplexity curve
    history_files = sorted(Path(config.model_dir).glob("training_history_*.json"), reverse=True)
    if history_files:
        try:
            ...
            visualizer.visualize_training_history(
                history,
                filename=f"perplexity_curve_{prefix}.png"
            )
            logger.info("Generated perplexity curve visualization")
        except Exception as e:
            logger.warning(f"Failed to generate perplexity curve: {e}")
