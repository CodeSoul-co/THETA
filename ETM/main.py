...
def run_evaluation(
    ...
    logger.info(f"  Topic Coherence (NPMI): {metrics['topic_coherence_avg']:.4f}")
...
def run_visualization(
    ...
    visualizer.visualize_topic_words(
        results['topic_words'],
        num_topics=config.visualization.num_topics_to_show,
        num_words=config.visualization.num_words_per_topic,
        as_wordcloud=use_wordcloud,
        filename=f"topic_words_{prefix}.png"
    )
    logger.info("Generated topic words visualization")
