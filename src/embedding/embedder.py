...
class QwenEmbedder:
    ...
    def embed_texts(
        ...
        if temp_dir is None:
            # Use data/workspace temp directory instead of src/embedding/outputs
            project_root = os.environ.get('PROJECT_ROOT', os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
            temp_dir = os.path.join(project_root, 'data', 'workspace', '.temp', f'.temp_{run_id}')
...
class EmbeddingManager:
    ...
    def __init__(
        ...
        # Default paths from environment - use workspace directory structure
        if output_dir is None:
            # Default to data/workspace/default_user/default_dataset/embeddings/
            project_root = os.environ.get('PROJECT_ROOT', os.path.dirname(os.path.dirname(os.path.dirname(__file__))))
            output_dir = os.path.join(project_root, 'data', 'workspace', 'default_user', 'default_dataset', 'embeddings')
