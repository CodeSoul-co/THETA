...
class DatasetLoader:
    ...
    def __init__(self, base_path: str = None, dev_mode: bool = False):
        # Default base path from environment or auto-detect
        if base_path is None:
            base_path = os.environ.get("PROJECT_ROOT")
            if not base_path:
                # Auto-detect: this file is at embedding/data_loader.py
...
def get_dataset_summary(base_path: str = None) -> pd.DataFrame:
    if base_path is None:
        base_path = os.environ.get('PROJECT_ROOT', str(Path(__file__).resolve().parent.parent))
    """
