    ...
    def __init__(
        ...
        **kwargs
        ...
        for width in self.hidden_sizes:
            encoder_layers.extend([nn.Linear(input_width, width), nn.Softplus(), nn.Dropout(dropout)])
            input_width = width
        self.encoder = nn.Sequential(*encoder_layers)
        
        # Latent parameters with batch norm
        self.mean_fc = nn.Linear(self.hidden_sizes[-1], num_topics)
        self.mean_bn = nn.BatchNorm1d(num_topics, affine=False)
        self.logvar_fc = nn.Linear(self.hidden_sizes[-1], num_topics)
