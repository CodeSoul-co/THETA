...
class DTMEncoder(nn.Module):
    ...
    def __init__(
        ...
        for width in self.hidden_sizes:
            encoder_layers.extend([nn.Linear(input_width, width), nn.ReLU(), nn.Dropout(dropout)])
            input_width = width
        self.doc_encoder = nn.Sequential(*encoder_layers)
        
        # Time encoding (learnable time embedding)
        self.time_embedding = nn.Embedding(time_slices, self.hidden_sizes[-1])
        
        # Fusion layer
        self.fusion = nn.Linear(self.hidden_sizes[-1] * 2, self.hidden_sizes[-1])
        
        # Output layers (VAE style)
        self.mu = nn.Linear(self.hidden_sizes[-1], num_topics)
        self.logvar = nn.Linear(self.hidden_sizes[-1], num_topics)
