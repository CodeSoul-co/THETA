...
def config_from_args(args: argparse.Namespace) -> PipelineConfig:
    ...
    hidden_sizes = getattr(args, "hidden_sizes", None)
    if hidden_sizes is not None:
        if not hidden_sizes or any(width <= 0 for width in hidden_sizes):
            raise ValueError("hidden_sizes must contain positive integers")
        config.model.hidden_sizes = list(hidden_sizes)
    config.model.encoder_dropout = _resolve("dropout", getattr(args, "dropout", None), param_type=float)
    ...
    resolved_kl_warmup = _resolve("kl_warmup", getattr(args, "kl_warmup", None), param_type=int)
    config.model.kl_warmup_epochs = 50 if resolved_kl_warmup is None else resolved_kl_warmup
    resolved_patience = _resolve("patience", getattr(args, "patience", None), param_type=int)
    config.model.patience = 10 if resolved_patience is None else resolved_patience
    ...
    # Zero-shot must use the prepared local/cloud vocabulary embeddings as-is.
    # Supervised and unsupervised modes retain the historical trainable default.
    if config.embedding.mode == "zero_shot" or (
        hasattr(args, "no_train_word_embeddings") and args.no_train_word_embeddings
    ):
