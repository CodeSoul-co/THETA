...
def generate_qwen_embeddings(
    ...
    from utils.local_embedding import encode_documents
    embeddings = encode_documents(texts, tokenizer, model, device, max_length, batch_size)
...
def prepare_baseline_data(args):
    ...
    if not args.bow_only and not args.skip_sbert:
        try:
    ...
    try:
        if df[time_column].dtype == 'object' or df[time_column].dtype == 'string' or pd.api.types.is_string_dtype(df[time_column]):
            ...
            def parse_chinese_date(date_str):
                # Parse each value independently: mixed slash/ISO/Chinese dates must
                # not silently discard the majority after one format succeeds.
                normalized = str(date_str).strip().replace('年', '-').replace('月', '-').replace('日', '').replace('/', '-')
                from datetime import datetime
                for pattern in ('%Y-%m-%d %H:%M:%S', '%Y-%m-%d %H:%M', '%Y-%m-%d', '%Y-%m', '%Y'):
                    try:
                        return datetime.strptime(normalized, pattern).year
                    except ValueError:
                        pass
                try:
                    return datetime.fromisoformat(normalized.replace('Z', '+00:00')).year
                except ValueError:
                    pass
            ...
            # Only use the explicitly selected time column. Modification time is
            # not a substitute for an article's missing publication date.
