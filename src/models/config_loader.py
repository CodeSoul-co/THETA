...
class EnvConfig:
    ...
    def load(cls, env_path: Optional[Path] = None):
        ...
        if env_path.exists():
            # Worker-owned task paths and approved runtime settings must win.
            # The repository .env is only a local fallback and may contain
            # placeholders that are invalid inside a container or job sandbox.
