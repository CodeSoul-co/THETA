...
def train_two_stage(
    ...
    for epoch in range(config.model.stage2_epochs):
        ...
        # KL annealing
        warmup_progress = 1.0 if config.model.kl_warmup_epochs == 0 else min(1.0, (epoch + 1) / config.model.kl_warmup_epochs)
...
def train_etm(
    ...
    for epoch in range(config.model.epochs):
        ...
        # Early stopping
        if config.model.early_stopping and not improved and patience_counter >= config.model.patience:
            if is_main_process(local_rank):
...
def save_results(
    ...
    save_trained_model(model, os.path.join(config.model_dir, 'delivery'), 'theta', vocab)
...
def run_train(config: PipelineConfig, logger: logging.Logger, local_rank: int = -1, world_size: int = 1):
    ...
    # Cloud/OpenAI-compatible embedding models may expose a configurable output
    # width. Keep the decoder aligned with the matrix preprocessing produced
    # instead of assuming the local Qwen width (1024).
    actual_word_dim = vocab_embeddings.shape[1]
    if config.model.word_embedding_dim != actual_word_dim:
        logger.info(
            "Auto-adjusting word_embedding_dim: "
            f"{config.model.word_embedding_dim} -> {actual_word_dim}"
        )
        config.model.word_embedding_dim = actual_word_dim
