...
def parse_args():
    ...
    parser.add_argument('--hidden_sizes', type=int, nargs='+', default=None,
                        help='Exact encoder widths, one positive integer per hidden layer')
...
def run_theta(args) -> Dict[str, Any]:
    ...
    train_config = {
        ...
        'hidden_sizes': args.hidden_sizes or [args.hidden_dim] * args.num_layers,
        'num_layers': len(args.hidden_sizes or [args.hidden_dim] * args.num_layers),
        ...
        'embedding_cloud_provider': args.embedding_cloud_provider,
    ...
    resolved_hidden_sizes = args.hidden_sizes or [args.hidden_dim] * args.num_layers
    cmd.extend(['--hidden_sizes', *[str(width) for width in resolved_hidden_sizes]])
...
def run_baseline(model_name: str, args) -> Dict[str, Any]:
    ...
    config = {
        ...
        'batch_size': args.batch_size,
        'hidden_dim': args.hidden_dim,
        'hidden_sizes': list(args.hidden_sizes or [args.hidden_dim] * args.num_layers),
        'num_layers': len(args.hidden_sizes or [args.hidden_dim] * args.num_layers),
        'dropout': args.dropout,
        'learning_rate': args.learning_rate,
        'patience': args.patience,
    ...
    if not args.skip_train:
        ...
        try:
            ...
            # Neural models
                ...
                hidden_sizes = tuple(args.hidden_sizes or [args.hidden_dim] * args.num_layers)
                train_result = trainer.train_ctm(inference_type=args.inference_type, epochs=args.epochs,
                                                 batch_size=args.batch_size, learning_rate=args.learning_rate,
                                                 hidden_sizes=hidden_sizes, dropout=args.dropout,
                                                 early_stopping_patience=args.patience,
                                                 early_stopping=not args.no_early_stopping)
            elif model_name == 'dtm':
                ...
                train_result = trainer.train_nvdm(epochs=args.epochs, batch_size=args.batch_size,
                                                  learning_rate=args.learning_rate, hidden_dim=args.hidden_dim,
                                                  hidden_sizes=args.hidden_sizes or [args.hidden_dim] * args.num_layers,
                                                  dropout=args.dropout, early_stopping_patience=args.patience,
                                                  early_stopping=not args.no_early_stopping)
            elif model_name == 'gsm':
        ...
        exported = export_baseline_metadata(model_dir, model_name, train_result, trainer)
        from model_delivery import save_trained_model
        # Exported metadata owns the fitted vocabulary (BERTopic's c-TF-IDF
        # vocabulary differs from the preprocessing BOW vocabulary).
        delivery_vocab = json.loads((model_dir / exported['beta']).with_name('vocab.json').read_text(encoding='utf-8'))
        save_trained_model(train_result['model'], model_dir / 'delivery', model_name, delivery_vocab)
